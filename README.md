...
- **Top offset when jumping from Outline Tree (pixels)**: extra space left above a line jumped to from the tree. `0` (the default) places the target line flush with the top of the editor; raise it when a sticky toolbar or theme header covers the top of the note. Maximum `1000`.
