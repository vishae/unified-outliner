...
上記修正の再デプロイ後、利用者による実機確認により、段落を隣接ブロックの下へ移動する操作で、区切り線が正しく移動先ブロックの下に表示されるようになったことが確認された。**この drop indicator の表示位置に関する問題は解消済みである。**

## 10. §6 実機受入テスト結果（利用者による手動確認、記録）

§6 の優先6操作について、利用者により Source Mode と Reading View の**両方**で手動確認が行われた。

- 結果: 6操作すべてにおいて、Tree の表示順序と本文の行順序に不一致は確認されなかった。
- §8 の受入基準に従い、これは「不一致は確認されなかった」という観測事実として記録する。「バグが存在しない」と断定するものではない。
- したがって、§5 に記載した3つの実機限定仮説（Reading View 再描画遅延・active leaf のずれ・fold/selection の錯覚）のうち、今回試行された範囲では、いずれも実際の不一致としては再現しなかった。
- 個別の操作ごとの再現回数・詳細な手順ログ（§7 テンプレートの粒度）は、今回は「全操作で異常なし」という総括的な報告として受け取ったものであり、操作別の内訳は記録していない。より厳密な記録が必要になった場合は、次回以降§7のテンプレートに沿って個別に記録する。
