...
## 10. Rename Interaction Contract の確定と優先度再評価（Phase 5T-13R、2026-08-24）

### 10-0 目的

5T-13D で指摘したコード上の懸念（id生成の非安定性、`renameSection`／
`renameListItem` における content-match／ambiguous-match の欠如）と、
§9で実機確認された interaction contract（blur先行効果）とを明確に区別
した上で、現在の優先度をロードマップへ正しく反映することを本節の目的
とする。

### 10-1 確認した事実（再掲）

§9 の実機確認により、次の4種の操作がいずれも rename textarea の focus
を外すことが確認されている。

- rename 中にツリー外のボタンを操作する
- rename 中に本文エディタへカーソルを移す
- rename 中に別 note／tab へ切り替える
- rename 中に Outline Tree の tab を閉じる

これらはすべて、既存の blur ハンドラを、当該操作本体の処理よりも前に
同期的に発火させ、rename を commit または終了させる。この前提のもと、
rename interaction contract を次のとおり確定する。

### 10-2 確定した Rename Interaction Contract

| 経路 | 契約内容 |
|---|---|
| Enter | 明示的 commit。内容が変更されていれば本文を更新し、未変更なら rename UI を閉じるのみとする。 |
| Escape | 明示的 cancel。通常 rename の本文は変更しない。ただし paragraph insert auto rename に限り、既存の安全条件（`canSafelyRollbackParagraphInsert` による再解決可否の確認）を満たす場合だけ insert 自体の rollback を試みる（この例外は変更せず維持する）。 |
| Blur（ツリー外へのフォーカス遷移） | 通常終了経路として正式に位置づける。内容が変更されていれば commit、未変更なら UI のみ閉じる。ユーザーが別作業へ移る前に rename を終える、意図した仕様であると明文化する。 |
| onClose（blur を経由しない close 経路） | blur を経由しない場合の保守的な fallback と位置づける。5T-11A で追加した `cancelRename()` 呼び出しは維持する。 |

上記は、5T-11A・5T-11D・5T-12A・5T-12D・5T-13D を通じて実装・確認済み
の既存挙動をそのまま追認するものであり、コード変更を伴わない。

### 10-3 5T-13D の結論訂正

以下を、5T-13D §3〜§5 の結論に対する訂正として明記する。

id 生成の非安定性、content-match の欠如、ambiguous-match の欠如は、
いずれも `edit/renameBlock.ts` 上のコード上の事実として変わらず存在
する。ただし、10-2 で確定した interaction contract のもとでは、通常の
対話 UI の経路で「rename 未確定のまま stale な rename state を維持し
つつ、同一ノートの本文を別操作で編集する」という前提そのものが成立
しない。したがって、案B（section/list への content-match／
ambiguous-match 導入）は、現在の利用者体験に対する緊急の修正課題では
ない。

これに伴い、手動 Method Vault マトリクスの S1〜S8（§3のリスクシナリオ
のうち、rename 未確定中の同一ノート内構造変更を要求するもの）は、実機
受入条件から外す。これらは、将来的に非同期の文書変更（他プラグインの
プログラム的編集、同期処理によるコンフリクト解決、フォーカスを奪わ
ない command 経由の操作など）が本プラグインまたは Obsidian 側に導入
された場合に、改めて検証対象とする。

「コード上の理論的な強化候補」と「実機で再現する不具合」は、以後明確
に区別して扱う。案B は前者に属し、後者ではない。

### 10-4 ロードマップ・Method Vault の扱い

- Phase 5T-13D は「完了（docs-only、コード監査完了）」と確定する。
- 案B（section/list への content-match＋ambiguous-match 導入、いわゆる
  「5T-13A」）は、直近の実装チケットへは進めず、将来の resolver
  hardening backlog へ移す。
- 優先度は、利用者に観測された実害、または非同期文書変更を伴う新機能
  の必要性が生じた時点で再評価する（10-5 参照）。
- Method Vault の S1〜S8 は「実機受入項目」から外し、「非同期変更機構
  導入時の将来検証候補」として位置づけを変更する。項目9〜11（正常系の
  回帰確認）は実機受入済みのまま維持する。項目12（commit後のUndo/
  Redo）は、S1〜S8 と同じ構造的理由により対象となる「誤反映commit」が
  発生し得ないため、S1〜S8 と同様に実機受入条件から外す。

### 10-5 将来再評価する条件

次のいずれかが明確に確認された場合、案Bおよび関連するMethod Vault
S1〜S8 の優先度を再評価する。

- rename 未確定中に、フォーカスを奪わない形で本文を書き換える他
  プラグイン・同期処理・スクリプトの存在が具体的に確認された場合。
- blur を発生させないキーボードショートカットやコマンドパレット経由
  の編集操作が、実機で確認された場合。
- 本プラグインまたは Obsidian 側に、非同期の文書変更（例: リアルタイム
  共同編集、バックグラウンド同期）を伴う新機能が導入される場合。
- 上記のいずれかに起因すると考えられる、利用者から報告された実害
  （誤った内容への commit 等）が発生した場合。

### 10-6 5T-12A note identity guard との関係の再確認

5T-12A の note identity guard（`evaluateRenameNoteIdentity`／
`abortRenameForNoteSwitch`）は、本節の結論によって不要になるものでは
ない。同 guard は、10-2 で確定した blur 先行効果という通常経路の不具合
を修正するために存在するのではなく、Obsidian のイベント順序という
非契約的な前提や、未検証のキーボード／コマンド経由操作、将来の実装
変更に対する安全網として、引き続き維持する。この位置づけは、案Bを
backlog へ移す本節の判断とは独立している。
